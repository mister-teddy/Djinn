...
export function Cave() {
	...
	return (
		<div className={ `flex h-[calc(100vh-32px)] items-stretch bg-white ${ resizing ? 'cursor-col-resize select-none' : '' }` }>
			<ToastHost />
			<div className="flex min-h-0 min-w-0 flex-none flex-col overflow-hidden" style={ { width: col.size } }>
				<div className="flex min-h-0 min-w-0 flex-none overflow-hidden" style={ { height: acc.size } }>
					<AccountTile />
				</div>
				<ResizeHandle axis="y" onMouseDown={ acc.startResize } title="Drag to resize" />
				<div className="flex min-h-0 min-w-0 flex-1 overflow-hidden">
					<CapabilitiesTile />
				</div>
			</div>
			<ResizeHandle axis="x" onMouseDown={ col.startResize } title="Drag to resize" />
