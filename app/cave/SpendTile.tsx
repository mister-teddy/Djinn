...
export function SpendTile() {
	...
	async function reset() {
		if (
			!window.confirm(
				'Reset the entire usage tally? This cannot be undone.',
			)
		) {
			return;
		}
	...
	const providerCol: Column<AnyRow> = {
		label: 'Provider',
		render: (r) => cap(r.provider || ''),
	};
	const modelCol: Column<AnyRow> = {
		label: 'Model',
		render: (r) => <code>{r.model}</code>,
	};
	const costCol: Column<AnyRow> = {
		label: onProxy ? 'Cost' : 'Est. cost',
	...
	return (
		<Tile title="Spend">
			...
			<Cards>
				...
				/>
				<StatCard
					value={t.completion.toLocaleString()}
					label="Output tokens"
				/>
			</Cards>
...
function DailyBars({ byDay }: { byDay: { day: string; cost: number }[] }) {
	...
	return (
		<div className="flex h-40 items-end gap-2 rounded-djinn bg-[#f5f5f7] p-4">
			{days.map((d) => {
				const pct =
					max > 0 ? Math.max(3, Math.round((d.cost / max) * 100)) : 3;
				return (
					<div
						key={d.key}
						className="flex h-full flex-1 flex-col items-center justify-end"
						title={`${d.key} — ${formatCost(d.cost)}`}
					>
						<div
							className="w-3/5 min-h-[3px] rounded-t bg-gradient-to-b from-gold to-gold-ember"
							style={{ height: pct + '%' }}
						/>
						<div className="mt-1.5 whitespace-nowrap text-[10px] text-[#787c82]">
							{d.key.slice(5)}
						</div>
					</div>
				);
