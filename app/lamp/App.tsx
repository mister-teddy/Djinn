...
import { config, streamWish, wish as wishApi, grant as grantApi, uploadFile as uploadFileApi, type Attachment } from '@shared/api';
import { Button, Spinner, Notice, Lamp, Sparkle, Popover, ResizeHandle, ToastHost, toast } from '@shared/ui';
...
export function App() {
	...
	useEffect( () => {
		...
		loadIndexStatus().then( setIndexInfo ).catch( () => {} );
	}, [] );
	...
	async function sendBlocking( startChatId: number, text: string, attachments: Attachment[] ) {
		const r = await wishApi( { chat_id: startChatId, message: text, attachments } );
	...
	async function send() {
		...
		setInput( '' );
		setAttachment( null );
		setMessages( ( m ) => [ ...m, { role: 'user', content: text, attachments: attachment ? attachments : undefined } ] );
		setBusy( true );
		setStep( '' );
		...
		try {
			...
			let terminal: { event: string; data: { chat_id?: number; message?: string } } | null = null;
			await streamWish( { chat_id: startChatId, message: text, attachments }, ( event, raw ) => {
				const data = ( raw || {} ) as { chat_id?: number; label?: string; token?: string; message?: string };
				if ( event === 'open' ) {
			...
			const term = terminal as { event: string; data: { message?: string } } | null;
			setError( term && term.event === 'error' ? term.data.message || 'The lamp dimmed.' : '' );
			if ( resolvedId ) {
	...
	function toggleSidebar() {
		setCollapsed( ( v ) => {
			const next = ! v;
			try {
				localStorage.setItem( 'djinn_sidebar_collapsed', next ? '1' : '0' );
			} catch {
				/* ignore */
			}
			return next;
	...
	async function resolvePending( pending: TranscriptMessage, confirmed: boolean ) {
		setBusy( true );
		try {
	...
	function indexPopover() {
		if ( indexInfo && indexInfo.embeds === false ) {
			return <div>This provider has no embeddings, so schema search runs on the full schema — no index needed.</div>;
		}
		const est = indexInfo?.estimate;
		const cost = est ? ( est.unpriced ? 'unknown' : est.free || est.cost === 0 ? 'free' : formatCost( est.cost ) ) : '…';
		const detail = est ? ` (~${ est.tokens.toLocaleString() } tokens · ${ est.chunks } chunks)` : '';
		return (
			<dl className="m-0 grid grid-cols-[auto_1fr] gap-x-3 gap-y-0.5">
				...
				<dd className="m-0 text-right">{ indexInfo ? String( indexInfo.countLive ) : '…' }</dd>
				<dt className="text-ivory-muted">Estimated cost</dt>
				<dd className="m-0 text-right">{ cost }{ detail }</dd>
			</dl>
		);
	}
		...
		return (
			<div className="flex h-full items-center justify-center bg-[radial-gradient(circle_at_50%_30%,rgba(251,191,36,0.16),transparent_50%),linear-gradient(135deg,var(--djinn-midnight),var(--djinn-violet))]">
				<div className="p-6 text-center text-ivory">
					<div className="flex justify-center"><Lamp size={ 96 } /></div>
					<h1 className="mb-1.5 mt-4 text-[26px]">The lamp is empty.</h1>
					<p className="mb-4 text-ivory-muted">Place an offering — an API key — to summon the Djinn.</p>
					<a className="inline-flex items-center rounded-control border-0 bg-gradient-to-b from-gold to-gold-deep px-4 py-2 font-semibold text-midnight shadow-glow" href={ config.settingsUrl }>Open the Cave of Wonders →</a>
				</div>
			</div>
		);
	}
	...
	const buildBtn = (
		<Popover key="build" placement="top" content={ indexPopover() }>
			<Button
				...
				className={ indexed ? `h-11 px-[18px] ${ ICON_BTN } w-auto text-[13px]` : 'h-11 px-[18px]' }
				busy={ indexing }
				onClick={ buildIndex }
			>
				{ ! indexed && <Sparkle /> }{ buildLabel }
			</Button>
		</Popover>
	);
		...
		>
			...
			<div className="flex min-h-0 min-w-0 flex-1 flex-col">
				<div className={ `flex flex-none items-center justify-between gap-4 px-[22px] py-4 ${ HEADER_BG }` }>
					...
					<div className="flex items-center gap-3"><Meter usage={ usage } /></div>
				</div>
				{ error && <Notice status="error">{ error }</Notice> }
				<div ref={ scroller } className={ `min-h-0 flex-1 overflow-y-auto p-[22px] [overscroll-behavior:contain] shadow-[inset_0_0_60px_rgba(0,0,0,0.35)] ${ THREAD_BG } ${ empty ? 'flex items-center justify-center' : '' }` }>
					{ empty ? (
						<div className="text-center text-ivory-muted">
							<div className="flex justify-center"><Lamp size={ 84 } glow /></div>
							<p className="mb-1 mt-4 text-[22px] font-medium tracking-wide text-ivory">Rub the lamp.</p>
							<p className="mx-auto max-w-[480px] text-[13px] text-ivory-muted">
								Try: <em className="not-italic text-gold">&quot;Create a draft page titled About&quot;</em> · <em className="not-italic text-gold">&quot;List my 5 newest posts&quot;</em> · <em className="not-italic text-gold">&quot;Set the tagline to Built with Djinn&quot;</em>
							</p>
						</div>
					) : (
						messages.map( ( msg, i ) => (
							<Message key={ i } msg={ msg } busy={ busy } onConfirm={ () => resolvePending( msg, true ) } onCancel={ () => resolvePending( msg, false ) } />
						) )
					) }
					{ busy && ! empty && (
				...
				<div className={ `flex flex-none flex-col items-stretch gap-2 px-4 py-3.5 bg-gradient-to-br from-midnight to-violet` }>
					...
					<div className="flex items-start gap-2.5">
						...
						/>
						<Button variant="tertiary" className={ ICON_BTN } disabled={ busy } title="Attach a file" onClick={ () => fileInput.current?.click() }>📎</Button>
						<textarea
							className={ INPUT_CLASS }
							value={ input }
							ref={ inputRef }
							placeholder={ indexed ? 'Whisper your wish…  (Enter to send · Shift+Enter for newline)' : 'Build the RAG index to begin granting wishes…' }
							rows={ 1 }
							disabled={ busy }
						...
						{ ! indexed ? (
							...
							<>
								{ stale && buildBtn }
								<Button key="send" variant="primary" className="h-11 px-[18px]" disabled={ busy || ( ! input.trim() && ! attachment ) } onClick={ send }>
									<Sparkle />Make wish
								</Button>
							</>
						) }
