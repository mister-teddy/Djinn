...
export const config: DjinnConfig = ( window.DjinnCave ?? window.Djinn ) as DjinnConfig;
...
export const gql = createClient( {
	fetcher: ( operation ) =>
		apiFetch( { url: config.gqlUrl, method: 'POST', data: operation } ) as Promise<any>,
} );
...
export function wish( body: WishBody ): Promise<{ status?: string; message?: string; chat_id?: number }> {
	return apiFetch( { url: `${ config.restUrl }/wish`, method: 'POST', data: body } ) as Promise<{ status?: string; message?: string; chat_id?: number }>;
}

/** Confirm or refuse a pending mutation; resumes the agent turn and returns its final result. */
export function grant( chatId: number, pendingId: number, confirmed: boolean ): Promise<{ status?: string; message?: string }> {
	return apiFetch( {
