...
function mdInline( text: string ): ReactNode[] {
	const patterns: { re: RegExp; make: ( m: RegExpExecArray ) => ReactNode }[] = [
		{ re: /`([^`]+)`/, make: ( m ) => h( 'code', { className: CODE }, m[ 1 ] ) },
		{
			re: /!\[([^\]]*)\]\(([^)\s]+)\)/,
			make: ( m ) => {
				const u = safeUrl( m[ 2 ] );
				return u ? h( 'img', { className: 'my-1.5 h-auto max-w-full rounded-lg border border-white/10', src: u, alt: m[ 1 ], loading: 'lazy' } ) : m[ 1 ];
			},
		},
		{
			re: /\[([^\]]+)\]\(([^)\s]+)\)/,
			make: ( m ) => {
				const u = safeUrl( m[ 2 ] );
				return u ? h( 'a', { href: u, target: '_blank', rel: 'noopener noreferrer', className: A }, ...mdInline( m[ 1 ] ) ) : m[ 0 ];
			},
		},
		{ re: /\*\*([^*]+)\*\*/, make: ( m ) => h( 'strong', null, ...mdInline( m[ 1 ] ) ) },
		{ re: /__([^_]+)__/, make: ( m ) => h( 'strong', null, ...mdInline( m[ 1 ] ) ) },
		{ re: /\*([^*]+)\*/, make: ( m ) => h( 'em', null, ...mdInline( m[ 1 ] ) ) },
		{ re: /_([^_]+)_/, make: ( m ) => h( 'em', null, ...mdInline( m[ 1 ] ) ) },
		{
			re: /https?:\/\/[^\s<>()[\]'"]*[^\s<>()[\]'".,;:!?]/,
			make: ( m ) => {
				const u = safeUrl( m[ 0 ] );
				return u ? h( 'a', { href: u, target: '_blank', rel: 'noopener noreferrer', className: A }, m[ 0 ] ) : m[ 0 ];
			},
		},
	];
	const out: ReactNode[] = [];
	...
	while ( rest ) {
		let best: { p: ( typeof patterns )[ number ]; m: RegExpExecArray } | null = null;
...
function mdTable( lines: string[], start: number ): { next: number; node: ReactNode } {
...
const H_SIZE: Record<number, string> = { 1: 'text-xl', 2: 'text-lg', 3: 'text-base', 4: 'text-sm', 5: 'text-sm', 6: 'text-sm' };
...
export function renderMarkdown( text: string ): ReactNode[] {
	...
	let i = 0;
	let key = 0;
	const isSpecial = ( l: string ) => /^```/.test( l ) || /^(#{1,6})\s/.test( l ) || /^\s*>/.test( l ) || /^\s*[-*+]\s+/.test( l ) || /^\s*\d+\.\s+/.test( l );
	while ( i < lines.length ) {
		...
		if ( hd ) {
			...
			blocks.push( h( `h${ lvl }`, { key: key++, className: `mb-1.5 mt-3 font-semibold leading-snug text-ivory ${ H_SIZE[ lvl ] }` }, ...mdInline( hd[ 2 ] ) ) );
			i++;
			continue;
		}
		if ( /^\s*([-*_])(\s*\1){2,}\s*$/.test( line ) ) {
			blocks.push( h( 'hr', { key: key++, className: 'my-3 border-0 border-t border-gold/20' } ) );
			i++;
			continue;
		}
		if ( /^\s*>/.test( line ) ) {
			...
			blocks.push( h( 'blockquote', { key: key++, className: 'my-2 border-l-[3px] border-gold/50 px-3 py-1 text-ivory-muted' }, ...mdInline( buf.join( '\n' ) ) ) );
			continue;
		}
		...
		if ( /^\s*[-*+]\s+/.test( line ) ) {
			...
			blocks.push( h( 'ul', { key: key++, className: 'my-1 mb-2 list-disc pl-[22px]' }, ...items.map( ( it, j ) => h( 'li', { key: j, className: 'my-0.5 leading-normal' }, ...mdInline( it ) ) ) ) );
			continue;
		}
		if ( /^\s*\d+\.\s+/.test( line ) ) {
			const items: string[] = [];
			...
			blocks.push( h( 'ol', { key: key++, className: 'my-1 mb-2 list-decimal pl-[22px]' }, ...items.map( ( it, j ) => h( 'li', { key: j, className: 'my-0.5 leading-normal' }, ...mdInline( it ) ) ) ) );
			continue;
		}
		const para = [ line ];
		i++;
		...
		blocks.push( h( 'p', { key: key++, className: 'mb-2 leading-relaxed' }, ...inline ) );
	}
	return blocks;
}
