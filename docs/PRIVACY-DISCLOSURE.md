...
**Managed proxy** — requests go to **Djinn's hosted proxy** at
**https://djinn-proxy-351601184057.asia-northeast1.run.app**, which forwards them to
**Google Gemini**, meters usage, and returns the response. The proxy is operated by
**Nguyễn Hồng Phát**; see the
...
**Bring your own key** — requests go **directly from your server** to the provider you configure
(**OpenAI**, **Google Gemini**, or **Anthropic**) using *your* API key.

Your data is also subject to the chosen provider's terms and privacy policy:
...
- Anthropic — commercial terms: https://www.anthropic.com/legal/commercial-terms ; privacy center: https://privacy.claude.com/en/
- Polar — used only for managed-proxy checkout/top-up billing; buyer terms: https://polar.sh/legal/checkout-buyer-terms ; privacy: https://polar.sh/legal/privacy
...
- **Managed proxy:** we retain only the **usage metadata** needed to meter and bill — token counts,
...
- Djinn only contacts an external service **when you make a wish** — it does not phone home in the
  background.
- The plugin shows a data-use notice in the admin, linking this disclosure.
...
> results of any read queries it runs to an LLM in order to fulfil your request. Managed proxy:
> via Djinn's hosted proxy (https://djinn-proxy-351601184057.asia-northeast1.run.app — terms:
> /terms, privacy: /privacy) to Google Gemini, with Polar used only for checkout/top-up billing.
> BYO key: directly to the provider you configure (OpenAI, Google Gemini, or Anthropic). Data is
> sent only when you make a wish. Provider policies are listed in the External services section.
